...
// below is a method to initiate various nodes !
...
-(id)initWithSize:(CGSize)size {
    ...
    if (self = [super initWithSize:size]) {
        ...
        // before adding inFrame, and referring to self and the frame referred to (frame class), it was an invalid method signature so there was an error, when we tried to use parent as a class to refer to, in which case there was no parent. 
        [self addChild:hud];
        
        //THHudNode is a subclass of SKNode, which starts from the elft edge of the screen.
...
- (void)touchesBegan:(NSSet<UITouch *> *)touches withEvent:(UIEvent *)event {
    ...
    for (UITouch *touch in touches) {
        ...
        // destroys current or previous scene in order to present the new game over scene.
    }
        
    
    // type casting allows the self ... part of the function to be returned as a type of SKNode which allows conversion into THSpaceCatNode type and allows the key part (THSpaceCatNode *spaceCat) to be compatible with each other.
}
...
- (void) shootProjectileTowardsPosition:(CGPoint)position {
    ...
    // what does add child mean?
    // What does child node mean?
    // it seems that the class "THProjectileNode" is being referred to when named as *projectile, and the characteristic if determine by using projectileAtPosition = position referred to
...
-(void) addSpaceDog {
    ...
    NSUInteger randomSpaceDog = [THUtil randomWithMin:1 max:3];
...
- (void) addPoints: (NSInteger)points {
    ...
    // incompatible points types mean you need to type cast with a * name.
}
...
- (void) loseLife {
    ...
    // Basically, this method returns whether there are any lives left or not.
...
- (void) createDebrisAtPosition:(CGPoint)position {
    ...
    for (int i = 0; i < numberOfPieces; i++) {
        ...
        // randomPiece is the passed in class of NSInteger named NSString called @"debri_%d".
